...
def heat_map(img, bbox_buffer, threshold=1):
    ...
    #heat *= 255.0/max(1, len(bbox_list))
    ...
    #print("heatmap:", heatmap.max(), heatmap.shape, heatmap.dtype)
    #heat_image = heat.astype(np.uint8)
    #print("heat_image:", heat_image.max(), heat_image.shape, heat_image.dtype)
    #heat_map = cv2.applyColorMap(heat_image, cv2.COLORMAP_HOT)
    #print("heat_map:", heat_map.max(), heat_map.shape, heat_map.dtype)

    # Combine the result with the original image
    #merge = cv2.addWeighted(img, 1, heat_map, 0.3, 0)
